...
std::uint64_t trailheadScore(      std::int32_t** data,
                             const std::int32_t   x,
                             const std::int32_t   y,
  ...
  if (*(*(data + y) + x) == 9) {
    ...
    for (std::int32_t step = -1; step <= 1; step += 2) {
      if ((x + step >= 0) && (x + step < maxX) &&
        ...
        sum += trailheadScore(data, x + step, y, maxX, maxY);
      }

      if ((y + step >= 0) && (y + step < maxY) &&
          (*(*(data + y + step) + x) == *(*(data + y) + x) + 1)) {
        sum += trailheadScore(data, x, y + step, maxX, maxY);
      }
    }
  }
...
int main() {
  ...
  for (std::int32_t y = 0; y < maxY; ++y) {
    for (std::int32_t x = 0; x < maxX; ++x) {
      if (*(*(data + y) + x) == 0) {
        sum += trailheadScore(data, x, y, maxX, maxY);
      }
    }
  }
